...
class Stats:
    ...
    def set_current_match(self, match_date: date, surface: Surface | None = None):
        assert match_date is not None and (
            self._last_match_date is None or match_date >= self._last_match_date
        )

        self._last_match_date = match_date
    ...
    def get_last_match_date(self) -> date | None:
        return self._last_match_date
    ...
    def get_player_overall_stats(self, player_id: int) -> PlayerMatchStats:
        if player_id not in self._player_overall_stats:
            ...
            self._player_overall_stats[player_id].update(self._last_match_date)
    ...
    def get_player_surface_stats(
        ...
        if (player_id, surface) not in self._player_surface_stats:
            ...
            self._player_surface_stats[(player_id, surface)].update(
                self._last_match_date
            )
    ...
    def get_elo(self, player_id: int, surface: Surface | None = None) -> float:
        if surface is None:
    ...
    def get_elo_delta(
        self, player1_id: int, player2_id: int, surface: Surface | None = None
    ) -> float:
        if surface is None:
    ...
    def get_welo(self, player_id: int, surface: Surface | None = None) -> float:
        if surface is None:
    ...
    def get_glicko(self, player_id: int, surface: Surface | None = None) -> float:
        if surface is None:
            return self.glicko.get_rating(player_id)

        return self.surface_glicko[surface].get_rating(player_id)

    def get_glicko_delta(
        self, player1_id: int, player2_id: int, surface: Surface | None = None
    ) -> float:
        if surface is None:
            return self.glicko.get_rating(player1_id) - self.glicko.get_rating(
                player2_id
            )
    ...
    def get_wglicko(self, player_id: int, surface: Surface | None = None) -> float:
        if surface is None:
            return self.wglicko.get_rating(player_id)

        return self.surface_wglicko[surface].get_rating(player_id)

    def get_wglicko_delta(
        self, player1_id: int, player2_id: int, surface: Surface | None = None
    ) -> float:
        if surface is None:
            return self.wglicko.get_rating(player1_id) - self.wglicko.get_rating(
                player2_id
            )
